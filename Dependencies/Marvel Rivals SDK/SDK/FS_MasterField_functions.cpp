#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: FS_MasterField

#include "Basic.hpp"

#include "FS_MasterField_classes.hpp"
#include "FS_MasterField_parameters.hpp"


namespace SDK
{

// Function FS_MasterField.FS_MasterField_C.时间轴__UpdateFunc
// (BlueprintEvent)

void AFS_MasterField_C::时间轴__UpdateFunc()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "时间轴__UpdateFunc");

	UObject::ProcessEvent(Func, nullptr);
}


// Function FS_MasterField.FS_MasterField_C.时间轴__FinishedFunc
// (BlueprintEvent)

void AFS_MasterField_C::时间轴__FinishedFunc()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "时间轴__FinishedFunc");

	UObject::ProcessEvent(Func, nullptr);
}


// Function FS_MasterField.FS_MasterField_C.UserConstructionScript
// (Event, Public, HasDefaults, BlueprintCallable, BlueprintEvent)

void AFS_MasterField_C::UserConstructionScript()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "UserConstructionScript");

	UObject::ProcessEvent(Func, nullptr);
}


// Function FS_MasterField.FS_MasterField_C.SetVisibility
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)

void AFS_MasterField_C::SetVisibility()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "SetVisibility");

	UObject::ProcessEvent(Func, nullptr);
}


// Function FS_MasterField.FS_MasterField_C.OnTriggerParticleFieldEvent
// (Event, Public, BlueprintEvent)
// Parameters:
// int32                                   ParticleIndex                                          (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void AFS_MasterField_C::OnTriggerParticleFieldEvent(int32 ParticleIndex)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "OnTriggerParticleFieldEvent");

	Params::FS_MasterField_C_OnTriggerParticleFieldEvent Parms{};

	Parms.ParticleIndex = ParticleIndex;

	UObject::ProcessEvent(Func, &Parms);
}


// Function FS_MasterField.FS_MasterField_C.OnTriggerEvent
// (Event, Public, BlueprintEvent)

void AFS_MasterField_C::OnTriggerEvent()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "OnTriggerEvent");

	UObject::ProcessEvent(Func, nullptr);
}


// Function FS_MasterField.FS_MasterField_C.ManualTrigger
// (Net, NetReliable, NetMulticast, BlueprintCallable, BlueprintEvent)

void AFS_MasterField_C::ManualTrigger()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "ManualTrigger");

	UObject::ProcessEvent(Func, nullptr);
}


// Function FS_MasterField.FS_MasterField_C.MakeDynamic_EnableNonGC
// (Public, BlueprintCallable, BlueprintEvent)

void AFS_MasterField_C::MakeDynamic_EnableNonGC()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "MakeDynamic_EnableNonGC");

	UObject::ProcessEvent(Func, nullptr);
}


// Function FS_MasterField.FS_MasterField_C.InitializeFieldVariables
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)

void AFS_MasterField_C::InitializeFieldVariables()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "InitializeFieldVariables");

	UObject::ProcessEvent(Func, nullptr);
}


// Function FS_MasterField.FS_MasterField_C.ForceMultiplier
// (Public, BlueprintCallable, BlueprintEvent)

void AFS_MasterField_C::ForceMultiplier()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "ForceMultiplier");

	UObject::ProcessEvent(Func, nullptr);
}


// Function FS_MasterField.FS_MasterField_C.FalloffShapeSwitch
// (Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// EFieldFalloffType                       falloffType                                            (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// const struct FVector2D&                 falloffMinMax                                          (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class UOperatorField**                  OperatorFieldOut                                       (Parm, OutParm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)

void AFS_MasterField_C::FalloffShapeSwitch(EFieldFalloffType falloffType, const struct FVector2D& falloffMinMax, class UOperatorField** OperatorFieldOut)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "FalloffShapeSwitch");

	Params::FS_MasterField_C_FalloffShapeSwitch Parms{};

	Parms.falloffType = falloffType;
	Parms.falloffMinMax = std::move(falloffMinMax);

	UObject::ProcessEvent(Func, &Parms);

	if (OperatorFieldOut != nullptr)
		*OperatorFieldOut = Parms.OperatorFieldOut;
}


// Function FS_MasterField.FS_MasterField_C.FalloffAndCullSwitch_Main
// (Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// double                                  Magnitude                                              (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// EFieldFalloffType                       FalloffType                                            (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// const struct FVector2D&                 FalloffMinMax_0                                        (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class UOperatorField*                   OperatorField                                          (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)
// class UCullingField**                   CullingField                                           (Parm, OutParm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)

void AFS_MasterField_C::FalloffAndCullSwitch_Main(double Magnitude, EFieldFalloffType FalloffType, const struct FVector2D& FalloffMinMax_0, class UOperatorField* OperatorField, class UCullingField** CullingField)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "FalloffAndCullSwitch_Main");

	Params::FS_MasterField_C_FalloffAndCullSwitch_Main Parms{};

	Parms.Magnitude = Magnitude;
	Parms.FalloffType = FalloffType;
	Parms.FalloffMinMax_0 = std::move(FalloffMinMax_0);
	Parms.OperatorField = OperatorField;

	UObject::ProcessEvent(Func, &Parms);

	if (CullingField != nullptr)
		*CullingField = Parms.CullingField;
}


// Function FS_MasterField.FS_MasterField_C.ExecuteUbergraph_FS_MasterField
// (Final, UbergraphFunction, HasDefaults)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void AFS_MasterField_C::ExecuteUbergraph_FS_MasterField(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "ExecuteUbergraph_FS_MasterField");

	Params::FS_MasterField_C_ExecuteUbergraph_FS_MasterField Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function FS_MasterField.FS_MasterField_C.DisplayTextSetup
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)

void AFS_MasterField_C::DisplayTextSetup()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "DisplayTextSetup");

	UObject::ProcessEvent(Func, nullptr);
}


// Function FS_MasterField.FS_MasterField_C.CE_Trigger
// (BlueprintCallable, BlueprintEvent)

void AFS_MasterField_C::CE_Trigger()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "CE_Trigger");

	UObject::ProcessEvent(Func, nullptr);
}


// Function FS_MasterField.FS_MasterField_C.CalculateNoise
// (Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// class UNoiseField**                     OutputPin                                              (Parm, OutParm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)

void AFS_MasterField_C::CalculateNoise(class UNoiseField** OutputPin)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FS_MasterField_C", "CalculateNoise");

	Params::FS_MasterField_C_CalculateNoise Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (OutputPin != nullptr)
		*OutputPin = Parms.OutputPin;
}

}

